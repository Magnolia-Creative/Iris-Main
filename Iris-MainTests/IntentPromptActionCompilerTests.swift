...
struct IntentPromptActionCompilerTests {
    ...
    @Test func llmPromptOmitsDeterministicAndEmbeddingHints() async throws {
        ...
        #expect(prompt?.contains("Action structure context:") == true)
        #expect(prompt?.contains("splitClip: {\"type\":\"splitClip\"") == true)
        #expect(prompt?.contains("\"target\":{\"type\":\"selectedClip|clipId|sameAsPrevious|ordinal|currentClipAtPlayhead\"") == true)
        #expect(prompt?.contains("target must be null or an object; never use a plain string target") == true)
